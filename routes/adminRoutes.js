...
router.get("/order/complete", isAuthenticatedAdmin, async (req, res, next) => {
    try {
...
router.get("/topThreeCategory", isAuthenticatedAdmin, async (req, res, next) => {
    try {
...
router.get("/topThreeMenu", isAuthenticatedAdmin, async (req, res, next) => {
    try {
        let orders = await COMPLETE_ORDERS.find({}).populate('menuId');
        let object = {};
        for (order of orders) {
